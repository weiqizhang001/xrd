...
# ----------------------------------
# F_CurveFit_Jade6: Pearson VII函数拟合曲线
# Input:    
#      number: 拟合点的个数
#      x: 拟合自变量
#      y: 拟合因变量
...
#       D: 半峰宽
#       yShow: 拟合后的y值
def F_CurveFit_Jade6(number, x, y, pattern):
    if pattern == 0:
...
# ----------------------------------
# F_CurveFit_Jade6_3Step: Pearson VII函数拟合曲线
# Input:    
#      number: 拟合点的个数
#      x: 拟合自变量
#      y: 拟合因变量
...
#       D: 半峰宽
#       yShow: 拟合后的y值
def F_CurveFit_Jade6_3Step(number, x, y, pattern):
    if pattern == 0:
        H, xCenter, D = optimize.curve_fit(F_PearsonVII_Jade, x, y)[0]  
        omega = 1.5
        yShow = H / np.power((1 + ((4 * (2 ** (1 / n) - 1)) / (D ** 2)) * np.power((x - xCenter), 2)), n) 
...
#PearsonVII Jade6
    ...
    yP = pathDataCruve[:, 1]
    number = np.size(xP) 
    x0, H, D, yShow = F_CurveFit_Jade6(number, xP, yP, int(sys.argv[3])) 
    H, xCenter, omega, sigma = optimize.curve_fit(F_PearsonVII, xP, yP)[0] 
    #F_ShowFittingPearsonVII(xP, yP, H, x0, omega, sigma)
    #omega = 1.5 fixed
    #H, xCenter, sigma = optimize.curve_fit(F_PearsonVII_15, xP, yP)[0]
...
#PearsonVII Jade6 3 steps
	...
	#new add 0602
    y_fit_5 = np.zeros(number-4)
    for i in np.arange(number-4):
        y_fit_5[i] = sum(yP[i:i+5]) / 5
	
	#min_forward, min_fw_index = 
    y_min_forward = np.min(y_fit_5[:int((number-4)/2)])
    y_min_backward = np.min(y_fit_5[int((number-4)/2):])
    if y_min_forward <= y_min_backward:
        y_min = y_min_forward * 0.85
    elif y_min_forward > y_min_backward:
        y_min = y_min_forward * 0.85
		
    y_fit_5_deduct = y_fit_5 - y_min
    x_fit_5 = xP[:number-4]
	
    x0, H, D, yShow = F_CurveFit_Jade6_3Step(number, x_fit_5, y_fit_5_deduct, int(sys.argv[3])) 
  
    omega = 1.5

    print(x0, H, D, omega, y_min, sep=',', end="\r\n")
